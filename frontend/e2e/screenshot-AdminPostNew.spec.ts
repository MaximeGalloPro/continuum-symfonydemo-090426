...
test('capture AdminPostNewPage', async ({ page }) => {
  await page.goto('/admin/posts/new')
  await page.waitForLoadState('networkidle')

  // Assertions: verify the page is functional (not showing errors)
  // 1. The page component should be visible
  await expect(page.getByTestId('admin-post-new-page')).toBeVisible({ timeout: 5000 })
  ...
  await page.screenshot({
    path: 'screenshots/AdminPostNewPage.png',
    fullPage: true
  })
})
